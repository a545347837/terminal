...
int main(int argc, char** argv) {
    ...
    ssl_opts.pem_root_certs  = CommonUtil::getContentFromFile(string("/home/config/client/server_self_signed_crt.pem"));
    ssl_opts.pem_private_key = CommonUtil::getContentFromFile(string("/home/config/client/client_privatekey.pem"));
    ssl_opts.pem_cert_chain  = CommonUtil::getContentFromFile(string("/home/config/client/client_self_signed_crt.pem"));
    std::shared_ptr<grpc::ChannelCredentials> creds = grpc::SslCredentials(ssl_opts);
    AppClient appClient(grpc::CreateChannel("localhost:50051", creds));
    cout<<"客户端已就绪"<<endl;
    while(1){
